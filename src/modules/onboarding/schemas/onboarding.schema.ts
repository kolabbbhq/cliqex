...
export const SignupSchema = z.object({
  ...
  primaryColor:  z.string().regex(/^#[0-9a-fA-F]{6}$/, 'Must be a valid hex color e.g. #1a8a5e').optional(),
...
export const ConfigureServicesSchema = z.object({
  ...
  })).min(1, 'Add at least one service').max(10),

  areas: z.array(z.object({
    id:    z.string().trim().min(1),
    label: z.string().trim().min(1).max(60),
  })).min(1, 'Add at least one delivery area').max(100),
...
export const UpdatePaymentDetailsSchema = z.object({
  ...
  bankAccountNumber: z.string().trim().regex(/^\d{10}$/, 'Must be exactly 10 digits'),
